...
impl Con {
    // Vox patch: upstream always requested PersistMode::Application but
    ...
    fn restore_token_path() -> std::path::PathBuf {
        ...
        base.join("vox").join("portal_restore_token")
    }

    fn load_restore_token() -> Option<String> {
        let contents = std::fs::read_to_string(Self::restore_token_path()).ok()?;
    ...
    async fn open_connection() -> ei::Context {
        ...
        if let Some(context) = ei::Context::connect_to_env().unwrap() {
            ...
            remote_desktop
                ...
                .unwrap();
            trace!("new session");
            let started = remote_desktop.start(&session, None).await.unwrap();
            if let Ok(selected) = started.response() {
